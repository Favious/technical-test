...
import FindReposPage from "pages/find_repos";
...
describe("FindUsersPage component", () => {
  ...
  it("should render the NavBar component", () => {
    const { getByText } = render(<FindReposPage />);
    expect(getByText(/FIND USERS/i)).toBeInTheDocument();
  });

  it("should render the SearchBar component", () => {
    const { getByPlaceholderText } = render(<FindReposPage />);
  ...
  it("should call the API when the search button is clicked", async () => {
    const { getByPlaceholderText, getByText } = render(<FindReposPage />);
